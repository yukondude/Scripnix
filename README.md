...
The following command descriptions were generated by `describe-scripnix` on November 2, 2016.
...
Usage: gather-cron-jobs [OPTIONS]
  ...
  consolidated table (tab-delimited by default): minute (m), hour (h), day
  of the month (dom), month (mon), day of the week (dow), user, and command.
  Optionally, the results may be sorted (as best as possible) by the
